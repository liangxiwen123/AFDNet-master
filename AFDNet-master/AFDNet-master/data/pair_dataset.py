...
class PairDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        h_1 = C_img.size(1)
        w_offset_1 = random.randint(0, max(0, w_1 - self.opt.fineSize - 1))
        
        h_offset_1 = random.randint(0, max(0, h_1 - self.opt.fineSize - 1))
        ...
        B_img = B_img[:, h_offset:h_offset + self.opt.fineSize,
               w_offset:w_offset + self.opt.fineSize]
        C_img = C_img[:, h_offset_1:h_offset_1 + self.opt.fineSize,
               w_offset_1:w_offset_1 + self.opt.fineSize]
        #print(C_img.shape)
        if self.opt.resize_or_crop == 'no':
            ...
            if (not self.opt.no_flip) and random.random() < 0.5:
                ...
                A_img_2 = A_img.index_select(2, idx)
            ...
            A_gray = 1. - (0.299*r+0.587*g+0.114*b)/2.
            A_gray = torch.unsqueeze(A_gray, 0)
        return {'A': A_img, 'B': B_img, 'C': C_img ,'A_gray': A_gray, 'input_img':input_img,
