...
def define_D(input_nc, ndf, which_model_netD,
    ...
    #elif which_model_netD == 'no_norm_4':
     #   netD = UNetDiscriminatorSN(input_nc, ndf)
    elif which_model_netD == 'no_norm_4':
        netD = NoNormDiscriminator(input_nc, ndf, n_layers_D, use_sigmoid=use_sigmoid, gpu_ids=gpu_ids)
...
class contrast(nn.Module):
    def __init__(self, nIn, nOut, stride=1, d=1):
        ...
        self.d2 = nn.Conv2d(nIn, nOut, kernel_size=3, stride=1, padding=6, bias=True, dilation=6)
...
class contrast1(nn.Module):
    def __init__(self, nIn, nOut, stride=1, d=1):
        ...
        self.d1 = nn.Conv2d(nIn, nOut, kernel_size=3, stride=1, padding=1, groups=nOut, bias=True, dilation=1)
        self.d2 = nn.Conv2d(nIn, nOut, kernel_size=3, stride=1, padding=12, bias=True, dilation=12)
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.act = nn.Sigmoid()
...
class ASPP(nn.Module):
    def __init__(self, in_channel=32, depth=32):
        super(ASPP, self).__init__()
        self.mean = nn.AdaptiveAvgPool2d((1, 1))
        self.conv = nn.Conv2d(in_channel, in_channel, kernel_size=(1, 1), stride=(1, 1))
        self.atrous_block1 = nn.Conv2d(in_channel, in_channel, kernel_size=(1, 1), stride=(1, 1))
        # 不同空洞率的卷积
        self.atrous_block6 = nn.Conv2d(in_channel, in_channel, kernel_size=(3, 3), stride=(1, 1), padding=(6, 6),
                                       dilation=(6, 6))
        self.atrous_block12 = nn.Conv2d(in_channel, in_channel, kernel_size=(3, 3), stride=(1, 1), padding=(12, 12),
                                        dilation=(12, 12))
        self.atrous_block18 = nn.Conv2d(in_channel, in_channel, kernel_size=(3, 3), stride=(1, 1), padding=(18, 18),
                                        dilation=(18, 18))
        self.conv_1x1_output = nn.Conv2d(in_channel * 5, in_channel, kernel_size=(1, 1), stride=(1, 1))
    ...
    def forward(self, x):
        ...
        atrous_block18 = self.atrous_block18(x)  # 不同空洞率的卷积

        x = torch.cat([image_features, atrous_block1, atrous_block6, atrous_block12, atrous_block18],
                      dim=1)  # 汇合所有尺度的特征
        # 利用1X1卷积融合特征输出
...
class Unet_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        ...
        self.conv2_1 = nn.Conv2d(35, 64, 3, padding=p)
        ...
        self.conv3_1 = nn.Conv2d(67, 128, 3, padding=p)
        ...
        self.conv4_1 = nn.Conv2d(131, 256, 3, padding=p)
        ...
        self.conv5_1 = nn.Conv2d(259, 512, 3, padding=p)
        ...
        self.conv6_1 = nn.Conv2d(768, 256, 3, padding=p)
        ...
        self.conv7_1 = nn.Conv2d(384, 128, 3, padding=p)
        ...
        self.conv8_1 = nn.Conv2d(192, 64, 3, padding=p)
        ...
        self.conv9_1 = nn.Conv2d(96, 32, 3, padding=p)
    ...
    def forward(self, input, gray):
        ...
        if self.opt.use_norm == 1:
            x_down2 = F.upsample(input, scale_factor=0.5, mode='bilinear')  # 160
            x_down4 = F.upsample(x_down2, scale_factor=0.5, mode='bilinear')
            x_down8 = F.upsample(x_down4, scale_factor=0.5, mode='bilinear')  # 160
            x_down16 = F.upsample(x_down8, scale_factor=0.5, mode='bilinear')

            x_reup8 = F.upsample(x_down16, scale_factor=2, mode='bilinear')  # 160
            x_reup4 = F.upsample(x_reup8, scale_factor=2, mode='bilinear')
            x_reup2 = F.upsample(x_reup4, scale_factor=2, mode='bilinear')  # 160
            x_reup = F.upsample(x_reup2, scale_factor=2, mode='bilinear')

            Laplace_8 = x_down8 - x_reup8
            Laplace_4 = x_down4 - x_reup4
            Laplace_2 = x_down2 - x_reup2
            Laplace_1 = input - x_reup
            if self.opt.self_attention:
                x = self.bn1_1(self.LReLU1_1(self.conv1_1(torch.cat((input,Laplace_1), 1))))
            ...
            if self.skip:
                if self.opt.linear_add:
                    ...
                    if self.opt.latent_threshold:
                        ...
                        latent = (latent - torch.min(latent)) / (torch.max(latent) - torch.min(latent))
                    output = latent + input * self.ratio
            else:
                output = latent
